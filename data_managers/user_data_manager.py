...
class UserDataManager:
    def __init__(self):
        self.db_path: str = os.path.normpath(
            os.path.join(os.path.dirname(__file__), '..', 'data', 'users_db.db'))
    ...
    def get_user_puzzle_statistics(self, user_id: int):
        query = (
    ...
    def update_user_settings(self, user_id: int, board_style: str, piece_style: str):
        query = (
            update(UserSettings)
    ...
    def update_user_puzzle_statistics(self, user_id: int, themes: str, solved: bool):
        query = (
            update(UserPuzzleStatistics)
            .where(UserPuzzleStatistics.user_id == user_id)
            .values(
